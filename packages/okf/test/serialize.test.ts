...
describe("serializeBundle (ElDoc superset)", () => {
  ...
  it("index uses the ElDoc columns", () => {
    expect(index).toContain("| Data Mart | Type | Storage |");
    expect(index).toContain("[Orders](./orders.md) | VIEW |");
  });

  it("mart frontmatter is ElDoc-shaped with no eldoc block", () => {
    expect(orders).toMatch(/type:\s*["']?ElDoc Data Mart["']?/);
    expect(orders).not.toContain("eldoc:");
    expect(orders).toContain("- eldoc");
    expect(orders).toContain("- view");
  });
