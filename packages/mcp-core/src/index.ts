    ...
    return {
      tools: [
          ...
          inputSchema: {
            ...
            properties: {
              graphJson: {
                type: "string",
                description:
                  "The JSON representation of the ElDoc model graph.",
              },
              dialect: {
                type: "string",
                description:
                  "The SQL dialect (e.g. 'postgres', 'snowflake', 'bigquery').",
                default: "postgres",
              },
            },
            required: ["graphJson"],
        ...
        },
        {
          name: "list_tables",
          description:
            "Lists all table/node names and descriptions in the model.",
          inputSchema: {
            type: "object",
            properties: {
              graphJson: {
        ...
        },
        {
          name: "describe_table",
          description:
            "Returns the schema (columns, types, roles) for a specific table.",
          inputSchema: {
            type: "object",
            properties: {
              graphJson: {
        ...
        },
        {
          name: "suggest_joins",
          description:
            "Analyzes schema fields and suggests potential Foreign Key connections.",
          inputSchema: {
            type: "object",
            properties: {
              graphJson: {
        ...
        },
        {
          ...
          description:
            "Adds a new table/node to the ElDoc model OKF file and saves it.",
          inputSchema: {
            type: "object",
            properties: {
              filePath: {
        ...
        },
        {
          ...
          description:
            "Adds a column to an existing table in the model.okf file.",
          inputSchema: {
            type: "object",
            properties: {
              filePath: {
        ...
        },
        {
          ...
          description:
            "Adds a foreign key edge between two tables in the model.okf file.",
          inputSchema: {
            type: "object",
            properties: {
              filePath: {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    if (request.params.name === "generate_sql") {
      ...
      try {
        ...
        return {
          content: [
            { type: "text", text: `Error generating SQL: ${err.message}` },
          ],
          isError: true,
        };
      }
      ...
      try {
        ...
        return {
          content: [
            { type: "text", text: text || "No business context found." },
          ],
        };
      } catch (err: any) {
        return {
      ...
      try {
        ...
        return {
          content: [
            { type: "text", text: `Error reading tables: ${err.message}` },
          ],
          isError: true,
        };
      }
      ...
      try {
        ...
        const table = graph.nodes.find(
          (n: any) =>
            n.title.toLowerCase() === tableName.toLowerCase() ||
            n.key === tableName,
        );
        if (!table) {
          return {
        ...
        return {
          content: [
            { type: "text", text: `Schema for ${table.title}:\n${schema}` },
          ],
        };
      } catch (err: any) {
        return {
          content: [
            { type: "text", text: `Error describing table: ${err.message}` },
          ],
          isError: true,
        };
      }
      ...
      try {
          ...
          for (const f of n.schema) {
            if (
              ...
              for (const pk of pks) {
                ...
                const targetName = pk.table.title
                  .toLowerCase()
        ...
        return {
          content: [
            { type: "text", text: `Error analyzing joins: ${err.message}` },
          ],
          isError: true,
        };
      }
      ...
      try {
        ...
        return {
          content: [
            { type: "text", text: `Table ${tableName} added to ${filePath}.` },
          ],
        };
      } catch (e: any) {
      ...
      const role = request.params.arguments?.role
        ? String(request.params.arguments?.role)
        : "none";
      ...
      try {
        ...
        const src = graph.nodes.find(
          (n: any) =>
            n.title.toLowerCase() === sourceTable.toLowerCase() ||
            n.key === sourceTable,
        );
        const tgt = graph.nodes.find(
          (n: any) =>
            n.title.toLowerCase() === targetTable.toLowerCase() ||
            n.key === targetTable,
        );
