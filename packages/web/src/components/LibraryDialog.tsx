...
function TemplateRow({
  ...
  onToggle,
  onUse,
}: { template: Template; open: boolean; onToggle: () => void; onUse: () => void }) {
  ...
  return (
    <div className="shrink-0 rounded-xl border border-[#e2e6ec] overflow-hidden">
      <div
        onClick={onToggle}
        role="button"
        className="flex items-center gap-3 px-4 py-3 hover:bg-[#f8fafc] text-left cursor-pointer"
      >
        {open ? (
        ...
        <span className="text-[11px] text-slate-500 whitespace-nowrap flex-shrink-0">
          {nodes.length} marts · {edges.length} links
        </span>
        <button
          onClick={(e) => {
            e.stopPropagation();
      ...
      {open && (
        <div
          className="px-4 pb-4 pt-1 bg-[#fbfcfe] border-t border-[#eef1f5] overflow-y-auto"
          style={{ maxHeight: "46vh" }}
        >
          <div className="flex flex-col gap-1.5 mt-2">
            {nodes.map((n) => (
              <MartRow key={n.key} title={n.title} fields={n.schema} />
            ))}
          </div>
                ...
                {edges.map((e) => {
                  ...
                  return (
                    <li key={e.id} className="flex items-center gap-2 text-[12px] text-slate-600">
                      ...
                      <span>
                        <b className="text-slate-800">{from}</b> {e.bidirectional ? "↔" : "→"}{" "}
                        <b className="text-slate-800">{to}</b>
                      </span>
          ...
          <tbody>
            {fields.map((f) => (
              <tr key={f.name} className="border-b border-[#f3f5f8] last:border-0">
                ...
                <td className="px-3 py-1.5 text-right text-[10.5px] text-[#1e88e5] font-semibold">
                  {f.role === "pk" ? "PK" : ""}
                </td>
              </tr>
            ))}
