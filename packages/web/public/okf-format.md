# ElDoc ERD Canvas — OKF authoring guide for AI agents

Use this to generate a data model in **OKF (Open Knowledge Format)** that imports cleanly into **ElDoc ERD Canvas** (https://model.eldoc.com) and pushes to ElDoc Data Marts in one click.

**How the user will use your output:** they open **Import** on the canvas, paste your text (or upload a `.zip`), review the rendered graph, then **Push to ElDoc**.
...
tags: ["eldoc", "sql"]          # 2nd tag = input source (lowercased)
---
# Orders
...
- `tags` — `["eldoc", "<input source lowercased>"]`, e.g. `["eldoc","view"]`.
...
- **CONNECTOR** — omit; configured in ElDoc after creation.
...
- **Cardinality (optional):** append `[1:1]`, `[1:N]`, `[N:1]` or `[N:N]` after the keys, oriented source → target — e.g. `` — `order_id = id` [N:1] ``. Visual only; ignored by ElDoc.
...
That's a complete, valid model: four marts, three relationships. The user pastes it into **Import**, picks a Storage, and clicks **Push to ElDoc**.
